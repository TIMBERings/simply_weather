angular.module('myApp.weather', [
	...
	.directive('daily', function(timeFactory, $sce) {
		return {
			scope: true,
			restrict: 'E',
			template: '<div class="col-xs-4 col-sm-4 col-md-4 col-lg-4" ng-bind-html="convertedDay">{{convertedDay}}</div>' +
				'<div class="weather_block daily col-xs-8 col-sm-8 col-md-8 col-lg-8">' +
				'<canvas id="weather_icon_{{$index}}" width="128" height="128"></canvas><br />' +
				...
				'<span>Moonphase: {{day.moonPhase}}</span>' +
				'</div>',
			link: function(scope) {
